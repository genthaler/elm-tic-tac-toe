...
This document defines the requirements for the single-screen tic-tac-toe application built with Elm. The app boots directly into the game, uses a shared theme module for light and dark color schemes, and relies on a web worker to keep AI calculations off the main UI thread. There is no landing page, routing layer, or style-guide/demo surface.
